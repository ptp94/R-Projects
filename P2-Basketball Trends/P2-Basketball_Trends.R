#Reference for data <- www.superdatascience.com
#----Dataset----
#Seasons
...
free.throws.mat <- rbind(KobeBryant_FT,JoeJohnson_FT,LeBronJames_FT,CarmeloAnthony_FT, DwightHoward_FT, ChrisBosh_FT,
                         ChrisPaul_FT, KevinDurant_FT, DerrickRose_FT, DwayneWade_FT)
rownames(free.throws.mat) <- Players
colnames(free.throws.mat) <- Seasons
free.throws.mat
...
#Matrix
free.throws.attempt.mat <- rbind(KobeBryant_FTA,JoeJohnson_FTA,LeBronJames_FTA,CarmeloAnthony_FTA, 
                                 DwightHoward_FTA, ChrisBosh_FTA,ChrisPaul_FTA, KevinDurant_FTA, 
                                 DerrickRose_FTA, DwayneWade_FTA)
rownames(free.throws.attempt.mat) <- Players
colnames(free.throws.attempt.mat) <- Seasons

free.throws.attempt.mat
...
#----Some Exploration of Players and Stats----
#How many games did LeBron James play in 2012
Games["LeBronJames", "2012"]

#Field Goals scored by him
FieldGoals["LeBronJames",]

#Goals Per Game
goals.per.game <- round(FieldGoals / Games, 2)
goals.per.game

#How many minutes per game did players play
minutes.per.game <- round(MinutesPlayed/Games, 2)
minutes.per.game

#We can see that Derrick Rose have an  NaN for 2012
minutes.per.game["DerrickRose", "2012"] <- 0
minutes.per.game

#Accuracy
accuracy.of.throws <- round(FieldGoals / FieldGoalAttempts, 2) * 100
accuracy.of.throws

#----Visualisation using Matplot----
#Creating a function for the plot to save time writing the code for each one.
plt.func <- function(data, rows = 1:10){
  Data <- data[rows, ,drop = F] #Convert to a matrix as matplot accepts that format
  matplot(t(Data), type = "b", pch = 15:18, col = c(1:4, 6))
  legend("bottomleft", inset =  0.01, legend = Players[rows], col = c(1:4, 6), pch = 15:18, horiz = F)
}

#Accuracy of Throws
plt.func(accuracy.of.throws)

#Minutes Played
plt.func(MinutesPlayed) #There are sharp drops as the players have been injured 

#Goals per Game for Kobe Bryant, LeBron James and Kevin Durant
plt.func(goals.per.game, c(1,3,8))

#Salary paid per point
plt.func(Salary/Points) #Kobe Bryant and Derrick Rose skewing the plot
plt.func(Salary/Points, c(-1,-9))

#Free Throws per game
plt.func(free.throws.attempt.mat / Games)

#Accuracy of Free throws
plt.func(free.throws.mat/free.throws.attempt.mat)

#Players playing style excluding free Throws 
plt.func((Points - free.throws.mat)/ FieldGoals)
